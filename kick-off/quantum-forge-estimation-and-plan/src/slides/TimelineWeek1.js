      ...
      <FlexBox height="100%" flexDirection="column" padding={isExportMode ? "40px" : "80px"} style={isExportMode ? { height: 'auto' } : {}}>
        <Box marginBottom={isExportMode ? "16px" : "32px"}>
          ...
          <Grid gridTemplateColumns="repeat(7, 1fr)" style={{ gap: isExportMode ? '2px' : '4px', marginBottom: isExportMode ? '16px' : '24px' }}>
            {[
              { day: 'Mon', date: '4', isHighlight: true },
              { day: 'Tue', date: '5' },
              { day: 'Wed', date: '6', isHighlight: true },
              { day: 'Thu', date: '7' },
              { day: 'Fri', date: '8', isHighlight: true },
              { day: 'Sat', date: '9' },
              { day: 'Sun', date: '10' }
            ].map((item, idx) => (
              <Box 
            ...
            <UnorderedList style={{ margin: '0px' }}>
              <ListItem>
                <Text fontSize="text" color="primary">
                  <strong>Monday (Aug 4):</strong> Discovery Session 1 (2-3 hours)
                </Text>
              </ListItem>
              <ListItem style={{ marginTop: '12px' }}>
                <Text fontSize="text" color="primary">
                  <strong>Wednesday (Aug 6):</strong> Discovery Session 2 (2-3 hours)
                </Text>
              </ListItem>
              <ListItem style={{ marginTop: '12px' }}>
                <Text fontSize="text" color="primary">
                  <strong>Friday (Aug 8):</strong> Discovery Session 3 + UX Review (2-3 hours)
                </Text>
              </ListItem>
            </UnorderedList>
            </Box>
